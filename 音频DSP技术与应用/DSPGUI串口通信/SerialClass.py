...
class SerialAchieve:
    ...
    def open_port(self,port):
        self.port = serial.Serial(port, self.bandRate,timeout = 0.5) # 没有连接超时
    ...
    def Read_data(self):   # self.port.read(self.port.in_waiting) 表示全部接收串口中的数据
        ...
        print('cout:',count)
        self.read_data =  self.port.read(count)   # 读取数据
        print(bytes(self.read_data).decode("ascii"))
        return bytes(self.read_data).decode("ascii")

    def Write_data(self,datas):
        if self.port.isOpen() == False:
            print("串口打开错误")
        else:
            self.port.write(datas.encode("ascii"))  # 返回的是写入的字节数
            print(datas.encode("ascii"))
